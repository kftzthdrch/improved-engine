...
@dataclass
class SetCabinTemperatureUseCase:
    ...
    def execute(self, vehicle_id: VehicleId, target_celsius: float) -> Command:
        now = self.clock.now()
        command = Command(
            id=self.id_gen.new_command_id(),
            vehicle_id=vehicle_id,
            command_type=CommandType.SET_CABIN_TEMPERATURE,
            status=CommandStatus.PENDING,
            created_at=now,
            updated_at=now,
            payload={"target_celsius": target_celsius},
        )
        try:
            self.policy.enforce_cabin_temperature_range(target_celsius)
        except Exception as e:
            command.mark_rejected(str(e), self.clock.now())
            self.command_repo.save(command)
            raise
        self.command_repo.save(command)
        result = self.gateway.send(vehicle_id, command.id, command.command_type, {"target_celsius": target_celsius})
        if result.success:
